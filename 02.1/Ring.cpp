...
void Ring::addNode (string desc, string data) {
	...
	if (AnzahlNodes == 0) {
		...
		if (AnzahlNodes == 1) {
			...
			if (AnzahlNodes > 0 && AnzahlNodes < 6) {
				// noch nicht voll
...
void Ring::get (string & search) {
	...
	for (int i = 0; i < 6 && tmp && (i == 0 || tmp != anker); i++) {
		if (tmp->getData() == search) {
			search = "OldAge " + to_string(tmp->getAge()) + ", Beschreibung: " + tmp->getDescription() + ", Daten: " +
				tmp->getData();
			break;
		}
		tmp = tmp->getNext();
...
void Ring::print () {
	RingNode* tmp = anker;
	for (int i = 0; i < 6 && tmp && (i == 0 || tmp != anker); i++) {
		cout << "OldAge: : " << tmp->getAge() << ", Descr: " << tmp->getDescription() << ", Data: " << tmp->getData() <<
			endl;
		cout << "--------------------------" << endl;
		tmp = tmp->getNext();
	}
